    ...
    private void MainWindowUI(int windowId)
    {
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label("Currency: ", GUILayout.Width(WINDOW_DIMENSION.x / 4f));
                    var currency = InventoryManager.Instance.availableCurrency;
                    currency = (int)GUILayout.HorizontalSlider(currency, 0f, 1000f, GUILayout.ExpandWidth(true));
                    if (GUI.changed)
                    {
                        InventoryManager.Instance.availableCurrency = currency;
                    }
                ...
                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label("Interactive mode: ", GUILayout.Width(WINDOW_DIMENSION.x / 4f));
                ...
                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label("Master volume: ", GUILayout.Width(WINDOW_DIMENSION.x / 4f));
                    var volume = SoundManager.Instance.masterVolume;
                    volume = GUILayout.HorizontalSlider(volume, -20f, 80f, GUILayout.ExpandWidth(true));
                    if (GUI.changed)
                    {
